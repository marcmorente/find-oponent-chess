...
function upload() {
  ...
  Swal.fire({
    ...
    if (result.value === null) {
      Swal.fire("Atenció!", "Has de seleccionar un arxiu.", "warning");
      return;
    }
    ...
    if (result.isConfirmed) {
      Swal.fire(
        ...
        !result.value.error
          ? "Ja pots buscar les partides al sistema."
...
function checkFile(validPGN, fileToUpload) {
  ...
  if (!validPGN.includes(fileToUpload.type)) {
    Swal.fire("Error", `${fileToUpload.name} no és un arxiu vàlid.`, "error");
    return false;
  }
